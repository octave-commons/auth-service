...
function parseAuthorizeResponse(locationHeader) {
...
function assertTokenResponse(body) {
...
function assertAccessTokenResponse(body) {
    if (body === null || typeof body !== 'object') {
...
test('authorization code flow issues and refreshes tokens', async (t) => {
    ...
    const authorizeResponse = await app.inject({
        method: 'GET',
        url: '/oauth/authorize?response_type=code&client_id=test-client&redirect_uri=https://example.com/cb&state=xyz&scope=read&code_challenge=' +
    ...
    t.true(tokenResponse.statusCode === 200 && Boolean(tokenBody.access_token && tokenBody.refresh_token));
...
//# sourceMappingURL=oauth-flow.test.js.map