...
async function handleAuthorizeRoute(req, reply) {
    const q = (req.query || {});
    if (q.response_type !== 'code') {
        return reply.code(400).send({ error: 'unsupported_response_type' });
    }
    const { client_id, redirect_uri, state, scope = '', code_challenge, code_challenge_method, login_hint, } = q;
...
function extractClientCredentials(body, req) {
...
function validateClient(client_id, client_secret, clients) {
...
function calculateGrantedScopes(requestedScope, clientScopes) {
...
async function handleClientCredentialsGrant(body, req, reply, clients) {
    ...
    const ttl = process.env.AUTH_TOKEN_TTL_SECONDS
        ...
        : 3600;
    const access_token = await signAccessToken({ sub: client_id, aud, scope: scopeStr, iss: ISSUER }, { expiresIn: ttl });
    return reply.send({
        access_token,
        token_type: 'Bearer',
        expires_in: ttl,
...
async function handleAuthorizationCodeGrant(body, reply) {
    ...
    const ttl = process.env.AUTH_TOKEN_TTL_SECONDS
        ? Number(process.env.AUTH_TOKEN_TTL_SECONDS)
        : 3600;
    const access_token = await signAccessToken({ sub: entry.sub, aud: client_id, scope: entry.scope, iss: ISSUER }, { expiresIn: ttl });
...
async function handleRefreshTokenGrant(body, reply) {
...
async function handleTokenRoute(req, reply) {
...
async function handleIntrospectionRoute(req, reply) {
    const ctype = (req.headers['content-type'] || '').toString();
    let body = {};
    const rawBody = req.body;
    if (ctype.includes('application/x-www-form-urlencoded') && typeof rawBody === 'string') {
